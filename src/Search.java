...
*******************************************************************************/
	...
	//	Set up Fitness Statistics matrix
		fitnessStats = new double[2][Parameters.generations][Parameters.numRuns];
		for (int i=0; i<Parameters.generations; i++){
			for (int j=0; j<Parameters.numRuns; j++){
				fitnessStats[0][i][j] = 0;
				fitnessStats[1][i][j] = 0;
			}
		}
		
		bestFitness = new double[Parameters.numRuns];
		bestGen = new double[Parameters.numRuns];
	...
	//	Initialize RNG, array sizes and other objects
		...
		for (R = 1; R <= Parameters.numRuns; R++){
			...
			for (G=0; G<Parameters.generations; G++){
				...
				fitnessStats[0][G][R-1] = sumRawFitness / Parameters.popSize;
				fitnessStats[1][G][R-1] = bestOfGenChromo.rawFitness;
...
*********************************************************************/
			...
			bestFitness[R-1] = (int)bestOfRunChromo.rawFitness;
			bestGen[R-1]= bestOfRunG;
		...
		summaryOutput.write("Gen          AvgFitMean          BestFitMean          AvgFitStdDev          BestFitStdDev\n");
		for (int i=0; i<Parameters.generations; i++){
			Hwrite.left(i, 15, summaryOutput);
			double avgFitMean = mean(fitnessStats[0][i]);
			double bestFitMean = mean(fitnessStats[1][i]);
			double avgFitStd = std(fitnessStats[0][i]);
			double bestFitStd = std(fitnessStats[1][i]);
			Hwrite.left(avgFitMean, 20, 2, summaryOutput);
			Hwrite.left(bestFitMean, 20, 2, summaryOutput);
			Hwrite.left(avgFitStd, 20, 2, summaryOutput);
			Hwrite.left(bestFitStd, 20, 2, summaryOutput);
			summaryOutput.write("\n");
		}
		
		summaryOutput.write("\n");
		
		summaryOutput.write("BestFitMean        BestFitStdDev        95%Conf.Interval\n");
		double bestFitMean = mean(bestFitness);
		double bestFitStdDev = std(bestFitness);
		double[] bestFitConfInterval = confint(bestFitness); //Change here
		Hwrite.left(bestFitMean, 20, 2, summaryOutput);
		Hwrite.left(bestFitStdDev, 20, 2, summaryOutput);
		Hwrite.left(bestFitConfInterval[0], 20, 2, summaryOutput); 
		Hwrite.left(bestFitConfInterval[1], 20, 2, summaryOutput); 
		summaryOutput.write("\n");
		summaryOutput.write("\n");
	
		summaryOutput.write("AvgOptGen  			OptGenStdDev 		95%Conf.IntervalOptGen\n");
		double bestGenMean = mean(bestGen);
		double bestGenStdDev = std(bestGen);
		double[] bestGenConfInterval = confint(bestGen);
		Hwrite.left(bestGenMean, 20, 2, summaryOutput);
		Hwrite.left(bestGenStdDev, 20, 2, summaryOutput);
		Hwrite.left(bestGenConfInterval[0], 20, 2, summaryOutput); 
		Hwrite.left(bestGenConfInterval[1], 20, 2, summaryOutput); 	
		summaryOutput.write("\n");	
	...
	private static double[] confint(double[] data) {
		double criticalValue = 1.96;
		double temp = criticalValue * std(data) / Math.sqrt(data.length);
		double[] results = new double[2];
		results[0] = mean(data) - temp;
		results[1] = mean(data) + temp;
		return results;
	}
